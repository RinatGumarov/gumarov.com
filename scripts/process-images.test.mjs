...
`,
    ...
    it('emits deterministic, metadata-free 4:5 images from a fixed centered crop', async () => {
      ...
      const cropSamples = await sharp(
        path.join(firstOutput, 'portrait-480.jpg'),
      )
      ...
      for (let index = 0; index < cropSamples.length; index += 3) {
        expect(cropSamples[index + 1] - cropSamples[index]).toBeGreaterThan(
          120,
        );
