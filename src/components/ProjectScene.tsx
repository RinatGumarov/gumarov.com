...
// The lead variant's frame runs the section's full content width instead of
// sharing it with a copy column, so it needs a wider hint.
const leadScreenshotSizes = '(min-width: 80rem) 1184px, 92vw';
...
export function ProjectScene({
  ...
  const isLead = project.variant === 'lead';
    ...
    >
      {showsScreenshot ? (
        <picture className={styles.screenshot}>
          <source
            type="image/avif"
            sizes={isLead ? leadScreenshotSizes : screenshotSizes}
  ...
  if (isLead) {
    return (
      ...
      >
        ...
        <figure className={styles.leadFigure} data-motion-reveal="visual">
          {visual}
